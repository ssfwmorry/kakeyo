...
// z.coerce.number().int().nonnegative() で整数・非負を保証する（方針確定書 §4.2）。

// 金額の上限（旧 utils/constants の MAX_PRICE）。旧 NotePrice.vue のテンキー入力は
// `model*10+num < MAX_PRICE` で桁上げを抑止していた＝「1000 万円未満」（実質の入力
// 上限は 9,999,999 円）。移植では priceSchema に上限を持たせ、金額を持つ全レーンへ
// 一括で効かせる。境界は旧に合わせ「未満」（10,000,000 は不可）とする。
...
export const priceSchema = z
  ...
  .pipe(
    z
      ...
      // 旧 MAX_PRICE の「未満」判定を踏襲（上限は 9,999,999 円）。
