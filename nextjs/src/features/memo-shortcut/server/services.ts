...
import * as demoMemoShortCut from '@/features/demo/server/queries/memo-shortcut';
...
export async function getMemoList(session: SessionData): Promise<MemoItem[]> {
  return withDemoRead(
    session,
    () => demoMemoShortCut.getMemoList(session),
...
export async function getShortCutList(
  ...
  return withDemoRead(
    session,
    () => demoMemoShortCut.getShortCutList(session),
