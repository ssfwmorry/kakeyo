...
export function MethodDialog({
  ...
  const [deleteResult, deleteAction] = useFormAction(deleteMethodAction);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        ...
        >
          ...
          <input type='hidden' name='payMode' value={payMode} readOnly />
          <input type='hidden' name='isPair' value={String(isPair)} readOnly />
          <FormField
            label={typeMethodLabels.entity.methodName}
            field={fields.name}
            key={editing?.id ?? 'new'}
          />
          <ColorPicker
