...
// 旧 utils/time.ts の DaysByMonth を移植する。月日指定は年を持たない（翌年に丸める）ため、
// 旧仕様どおり 2 月は常に 28 日固定（うるう年は非考慮）とする。
// ＝ これにより「4/31」「2/30」等の存在しない月日を選べなくする（現行は 1〜31 固定だった）。

// 各月（1-indexed）の日数（2 月は 28 固定）。
...
// month（1〜12）に存在する「日」の配列 [1..末日] を返す。
