...
// get_record_list の 1 行（カレンダー表示用）。id は record_id を採用、
// type_name の '精算' 補完・isInstead/isSettlement 導出済み。
...
// get_record_list とほぼ同形だが isSettlement を持たない。
...
// scope 外・不存在は null（呼び出し側で新規扱いにする）。
