...
- **凡例**: 🔴=UX の明確な後退（優先対応） / 🟡=見た目の後退・質感低下 / 🔵=要確認（実機/デザイン判断が要る）
...
### G-1. 共有(ペア)の視覚シグナルが「人物アイコン統一」から「色ドット/文字」へ退化 ★要望の核心
...
### U-1. 金額の電卓（テンキー）入力が完全廃止 → 素の1行テキスト入力 ★最優先
...
### U-4. カレンダー画面：FullCalendar のカスタム CSS 未移植で「素の FullCalendar」化 ★見た目が別物
