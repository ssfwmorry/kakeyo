...
import * as demoSummary from '@/features/demo/server/queries/summary';
import {
  getMethodCardList,
  getTypeCardList
...
export async function getTypeSummary(
  ...
  return withDemoRead(
    session,
    () => demoSummary.getTypeSummary(session, query),
    async () => {
...
export async function getMethodSummary(
  ...
  return withDemoRead(
    session,
    () => demoSummary.getMethodSummary(session, query),
    async () => {
...
export async function getPayAndIncomeList(
  ...
  return withDemoRead(
    session,
    () => demoSummary.getPayAndIncomeList(session, input),
    async () => {
...
export async function getTypeSummaryPeriod(
  ...
  return withDemoRead(
    session,
    () => demoSummary.getTypeSummaryPeriod(session, query),
    async () => {
...
export async function getSubTypeSummary(
  ...
  return withDemoRead(
    session,
    () => demoSummary.getSubTypeSummary(session, input),
    async () => {
...
// デモは getTypeCardList がモック（solo / pair 出し分け済み）を返すため、ここで別途注入しない。
export async function getTypeChips(
  ...
  const toChips = (cards: { id: number; name: string; colorName: string }[]) =>
    cards.map((card) => ({
      typeId: card.id,
      name: card.name,
