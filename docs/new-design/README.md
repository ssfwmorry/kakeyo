...
│   └── v2/               ← 新デザインのルート
│       ├── layout.tsx
│       ├── (tabs)/calendar/ summary/ bank/ setting/ ...
...
- [x] **T12. 入力フロー**（`NoteType` → `NoteSub` → `Note` / `NoteIncome` / `NotePair` / `NoteTypePair`）
  デザインは全画面モーダルだが、予定と同じくカレンダーの上に出るシートにした
  （`v2/features/note/components/record-sheet.tsx`）。スワイプで閉じられ、カレンダーに
  留まったまま登録できる。カテゴリ → サブカテゴリ → 金額と詳細の 3 画面を 1 つのシートの
  中で切り替える（シートの上にシートを重ねない。日付の暦もチップの下に開く）。
  タブバー中央の ＋ は `/v2/calendar?note=new` へ送り、カレンダー側でシートを開く。
  共有モードは「だれのお金で払った？」（立替／共有のお金）とメモ必須。
  「いつもの」は既存のショートカット（個人資産）を今のモードで登録できるものに絞って出す。
  旧カレンダーの「ショートカット記録」はこれで v2 に取り込んだ。
  スキーマ・サービスは旧 record のものを再利用し、遷移せず toast を返す薄い Action
...
- [x] **T13. 予定・記録の編集**（`PlanAdd` / `PlanEdit` / `RecordEdit`）
  ...
  記録の編集は同じシート。日別リストの記録行から開き、金額の画面から始まる。ピルで
  カテゴリを選び直せる。共有か個人かは対象に従い、個人｜共有は固定。下端に削除。
  編集中は「いつもの」を出さない（別の記録の内容で上書きされるため）。
  精算の記録（カテゴリ無し）は入力フローの形に載らないので押せない。
